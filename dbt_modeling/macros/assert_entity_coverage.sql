...
--
-- Returns expected entities (from the generated `expected_entities` seed,
-- filtered to `source_name`) that have NO rows in `model`. A test fails when any
-- come back — an expected symbol/series that was never ingested or dropped from
-- the universe entirely. Per-entity recency can't catch this: it only ranges
-- over entities that already exist in the data.
with expected as (
